...
# key is required when using some of the functions from Flask
...
def edit_record(serviceReport_id):
    if request.method == "POST":
        ...
        mongo.db.serviceReports.update(
            {"_id": ObjectId(serviceReport_id)}, submit)
        flash("Record Successfully Updated")

    serviceReport = mongo.db.serviceReports.find_one(
        {"_id": ObjectId(serviceReport_id)})
    serviceReports = list(mongo.db.serviceReports.find())
